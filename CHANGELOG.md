...
- Windows 经典右键菜单改为每用户 COM shell extension：注册 `HKCU\\Software\\Classes\\CLSID` 及文件/目录 `ContextMenuHandlers`，由 `IContextMenu` 动态生成父菜单、五种格式命令和预设组。该布局在 Windows Explorer 与 Directory Opus 13.25 x64 实机均可展开并执行，注册、配置、移除和使用均不需要 UAC；旧 HKLM/静态级联仅保留兼容清理路径。
